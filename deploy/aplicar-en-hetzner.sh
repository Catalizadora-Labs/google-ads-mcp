...
if echo "$R" | grep -q '8813881007' && ! echo "$R" | grep -q '"isError":true'; then echo "✓ search_execute_query contesta (page_size ya no se manda, API v24)"; else echo "✗ la GAQL falló: $(echo "$R" | cut -c1-300)"; exit 1; fi
